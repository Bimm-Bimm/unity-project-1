...
public class NetworkCharacterControllerPrototypeCustom : NetworkTransform {
  ...
  public virtual void Move(Vector3 direction) {
    ...
    {
        transform.Rotate( 0 , rotY * Runner.DeltaTime * 400, 0);
    }
    public virtual void MoveTo(Vector3 newPos) 
    {
        var prePos = transform.position;
        var dir = newPos - prePos;
        Controller.Move(dir);
        Velocity = Vector3.zero;
        IsGrounded = Controller.isGrounded;
    }
    public void MoveToward(Vector3 destination, float time) 
    {
        StartCoroutine(Lerp(destination , time));
    }
    IEnumerator Lerp(Vector3 destination , float time)
    {
        float timeElased = 0;
        while (timeElased< time) 
        {
            yield return new WaitForEndOfFrame();
            MoveTo(Vector3.Lerp(transform.position, destination, timeElased / time));
